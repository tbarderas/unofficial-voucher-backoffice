...
class VoucherTemplatesList extends Component {
    ...
    componentDidMount() {
        ...
        fetch('/vouplaVoucherTemplate/search/customSearchWithFilter?sort=desc')
            .then(response => response.json())
            .then(data => this.setState({voucherTemplates: data["_embedded"]["vouplaVoucherTemplate"], isLoading: false}));
    }

    async remove(id) {
        await fetch(`/vouplaVoucherTemplate/${id}`, {
            method: 'DELETE',
            headers: {
            ...
            let updatedVoucherTemplates = [...this.state.voucherTemplates].filter(i => i.templateId !== id);
            this.setState({voucherTemplates: updatedVoucherTemplates});
        });
    }
            ...
            return <tr key={voucherTemplate.templateId}>
                <td>
                    <ButtonGroup>
                        ...
                        <Button size="sm" color="danger" onClick={() => this.remove(voucherTemplate.templateId)}>
