...
const VoucherRulesList = () => {
    ...
    const renderRow = (voucherRuleOther) => (
        <tr key={voucherRuleOther.ruleId}>
            <td>
                <ButtonGroup>
                    <Button size="sm" color="primary" tag={Link} to={`/voucherRulesOthers/${voucherRuleOther.ruleId}`}>
                    ...
                    <Button size="sm" color="info" tag={Link}
                            to={`/voucherRulesOthers/${voucherRuleOther.ruleId}?clone`}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
